...
/**
 *   3. 分布式锁的实现细节
 *   获取分布式锁的重点逻辑在于 BaseDistributedLock，实现了基于Zookeeper实现分布式锁的细节。
 */
public class BaseDistributedLock {
    ...
    public BaseDistributedLock(ZkClient client, String path, String lockName) {
        this.client = client;
        this.basePath = path;
        this.path = path.concat("/").concat(lockName); // 临时节点： /lock/lock-
    ...
    protected String attemptLock(long time, TimeUnit unit) throws Exception {
        ...
        while (!isDone) {
            ...
            try {
                ...
                if (retryCount++ < MAX_RETRY_COUNT) { // 获取锁，最大重试次数
                    isDone = false;
                } else {
                    throw e;
                }
